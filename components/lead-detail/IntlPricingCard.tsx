...
import { Copy, DollarSign, Lock, TrendingDown, Hammer, ChevronDown, ChevronUp } from "lucide-react";
import type { PricingRecommendation } from "@/types";
import type { CountryCurrency } from "@/lib/intl-pricing";

interface IntlPricing extends PricingRecommendation {
  currency: CountryCurrency;
  localProjectPrice: number;
  localMonthlyPrice: number;
  localAnnualPrice: number;
  localFloorPrice: number;
  localDevMin: number;
  localDevMax: number;
  localDevFloor: number;
}

export function IntlPricingCard({ pricing }: { pricing: IntlPricing }) {
  ...
  const devCostPitch = pricing.projectDev
    ? `The ${pricing.projectDev.label.toLowerCase()} is a one-time project starting from ${fmt(pricing.localDevMin)} — fully custom-built for your business. Monthly platform subscription of ${fmt(pricing.localMonthlyPrice)}/month starts after delivery.`
    : null;
      ...
      <div className="rounded-xl border-2 overflow-hidden" style={{ borderColor: tc.border }}>
        <div className="flex items-center justify-between px-4 py-3" style={{ background: tc.bg }}>
          <div className="flex items-center gap-2">
            <DollarSign className="w-4 h-4" style={{ color: tc.text }} />
          ...
          >
            ...
            {showFloor ? (
              <span className="text-red-600 font-bold flex items-center gap-1">
                ...
                Floor: {fmt(pricing.localFloorPrice)}/mo
              </span>
            ) : "Show floor price"}
          </button>
        </div>
        ...
        <div className="rounded-xl border-2 border-purple-200 overflow-hidden">
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <span className="text-xl font-extrabold text-purple-700">
                {fmt(pricing.localDevMin)}–{fmt(pricing.localDevMax)}
              </span>
          ...
          {devExpanded && (
            <>
              ...
              <div className="px-4 py-3 bg-white border-b">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Payment Structure</p>
                <div className="space-y-1 text-sm text-gray-700">
                  <div className="flex items-center gap-2">
                    ...
                    <span><strong>50% upfront</strong> ({fmt(Math.round(pricing.localDevMin * 0.5))}) to start development</span>
                  </div>
                  <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2 mt-2 text-xs text-gray-500">
                    ...
                    <span>Monthly SaaS subscription of {fmt(pricing.localMonthlyPrice)}/mo starts after go-live</span>
                  </div>
                </div>
              </div>
              ...
              <div className="px-4 py-3 bg-gray-50 flex items-center justify-between">
                ...
                >
                  ...
                  {showDevFloor ? (
                    <span className="text-red-600 font-bold flex items-center gap-1">
                      <TrendingDown className="w-3 h-3" />
                      Floor: {fmt(pricing.localDevFloor)}
                    </span>
                  ) : "Show dev floor"}
                </button>
              </div>
