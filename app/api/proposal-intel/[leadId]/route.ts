...
import type { ProposalIntelData } from "@/types";
...
export async function GET(
  ...
  try {
    ...
    let cached;
    try {
      cached = await prisma.proposalIntelCache.findUnique({ where: { leadId } });
    } catch {
      await ensureTable();
    ...
    const result: ProposalIntelData = {
      ...
      ...(cached.data as unknown as Omit<ProposalIntelData, "leadId" | "generatedAt">),
    };

    return Response.json({ success: true, data: result });
  } catch (error) {
    console.error("[GET /api/proposal-intel/[leadId]]", error);
    return Response.json({ success: true, data: null });
  }
}
