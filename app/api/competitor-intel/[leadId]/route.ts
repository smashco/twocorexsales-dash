...
import type { CompetitorIntelData } from "@/types";
...
export async function GET(
  ...
  try {
    ...
    let cached;
    try {
      cached = await prisma.competitorIntelCache.findUnique({ where: { leadId } });
    } catch {
      await ensureTable();
    ...
    const result: CompetitorIntelData = {
      ...
      ...(cached.data as unknown as Omit<CompetitorIntelData, "leadId" | "generatedAt">),
    };

    return Response.json({ success: true, data: result });
  } catch (error) {
    console.error("[GET /api/competitor-intel/[leadId]]", error);
    return Response.json({ success: true, data: null });
  }
}
